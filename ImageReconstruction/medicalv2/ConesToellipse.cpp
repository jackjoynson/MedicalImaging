    ...
    _plane(plane)
{
    ...
    for(int i = 0; i < _cones.size(); i++){
        ...
        if(round(sin(omega)) == 1 || round(sin(omega)) == -1){
            phi2 = atan(fabs(thisCone.GetYImagePlane()/a));
        }
        else if(round(cos(omega)) == 1 || round(cos(omega)) == -1){
            phi2 = atan(fabs(thisCone.GetXImagePlane()/a));
            bot++;
        }

        double phi1 = theta - phi2;
        double u = fabs(a*tan(phi1));
        double v = fabs(a*tan(phi2));
        double rmin = u+v;
        double rmax;
        double majorRadius;
        double minorRadius;
...
//        }
        ...
        if(phi2 >= theta){
            double alpha = phi2-theta;
            double extraBit = fabs(a*tan(alpha));
            rmin = v-extraBit;
            rmax = fabs(a*tan(phi2+theta)) - extraBit-v;
            majorRadius = (rmax+rmin)/2.0;           //These are the SEMI major and MINOR axis radiuses;
            minorRadius = sqrt(rmax*rmin);
        ...
        else      //Change this later
        {
            ...
            {
                double vPlusT = fabs(a*tan(phi2 + theta));
                double T = vPlusT - v;
                rmax = vPlusT - v;
