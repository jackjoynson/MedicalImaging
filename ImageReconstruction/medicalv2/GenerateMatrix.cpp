...
vector<vector<int> > GenerateMatrix::GetValues(vector<Ellipse> ellipses, double width, double height, int matrixIntervals)
{
	...
	for (size_t ellipseIndex = 0; ellipseIndex < ellipses.size(); ellipseIndex++)
	{
        ...
        int MajorAxisX = round(ellipse.GetMajorAxis().getX());
        int MajorAxisY = round(ellipse.GetMajorAxis().getY());
		...
		for (int thetaStep = 0; thetaStep < thetaIntervals; thetaStep++)
		{
            ...
            if(MajorAxisX == 1 || MajorAxisX == -1){
                 radius = a*b/sqrt(pow(b*sin(theta), 2.0) + pow(a*cos(theta), 2.0));      //Flipped these
            }
            else if(MajorAxisY == 1 || MajorAxisY == -1){
                 radius = a*b/sqrt(pow(a*sin(theta), 2.0) + pow(b*cos(theta), 2.0));
            }

			double x = center.getX() + sin(theta)*radius;
			double y = center.getY() + cos(theta)*radius;

			int xIndex = round((x + xOffset)*matrixIntervals / width);
			int yIndex = round((y + yOffset)*matrixIntervals / height);
